...
class RLlibWrapperTests(unittest.TestCase):
    def test_observe_contains_action_mask_and_wrapped_observation(self):
        env = sternhalma_v0.rllib_env(num_players=2, board_diagonal=5, render_mode=None, max_actions=16)
        env.reset()
    ...
    def test_action_index_maps_to_first_valid_move(self):
        wrapped = sternhalma_v0.rllib_env(num_players=2, board_diagonal=5, render_mode=None, max_actions=32)
    ...
    def test_invalid_action_index_gets_penalty(self):
        env = sternhalma_v0.rllib_env(num_players=2, board_diagonal=5, render_mode=None, max_actions=8)
        env.reset()
